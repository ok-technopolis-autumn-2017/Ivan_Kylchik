...
function TodoActionsBarConstructor() {
    ...
    this._counterNode = document.querySelector('.todos-status-bar_counter');
    this._counterNodeText = getTextNode(this._counterNode);
    this._clearCompletedNode = document.querySelector('.todos-status-bar_delete-completed');

    this._clearCompletedNode.addEventListener('click', this);

    this._filters = new Filter(document.querySelector('.filters'));
